...
artisan ICPs — the evidence decides, not the fashion); LinkedIn-only for
pure credibility plays; one channel deep when starting. Cadence: steady
volume at cruise (~200-250 contacts/week), multichannel sequence (invite
→ email → follow-ups → DM → breakup). Per-tier treatment: **A** =
hot/manual with a mini-audit per account, **B** = standard sequence,
**C** = light touch or none. Forbidden: spray-and-pray, inbound-only
waiting, talking to off-ICP accounts because they are easy.

## The GO — one block, one confirmation

Present ≤ 12 lines: the evidence summary (coverage numbers, tiers, fresh
signals) → recommended motion + channel mix + cadence + per-tier
treatment → what it means concretely ("14 contacts: 9 linkedin+email, 5
email-only; the 3 tier-A get the manual treatment") → the phase-0
questions still open. ONE GO. Persisted; re-proposed only when
`context/` or the evidence shifts materially (new tiers, a big signal
wave, a channel's coverage doubling).
...
- **`contacts.channel_plan`** = `email` | `linkedin` | `linkedin+email` |
  ...
  linkedin_url? tier?) via `db.py modify --updates`, batched. Contacts of
